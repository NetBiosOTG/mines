...
async def mines_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle the /mines command - using simple URL button to avoid WebApp issues"""
    try:
        # Use simple URL button instead of WebApp to avoid the Button_type_invalid error
        keyboard = [
        ...
        logger.info(f"Mines command executed successfully for user {update.effective_user.id}")
        
    except BadRequest as e:
        logger.error(f"BadRequest error in mines_command: {e}")
        # If button fails, send plain text with URL
        await update.message.reply_text(
            f"🎮 *Mines Multiplier Calculator*\n\n"
            f"Calculate your potential winnings for Mines game!\n\n"
            f"Open the calculator here: {WEBAPP_URL}",
            parse_mode='Markdown'
        )
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Try to send error message if we have an update with a message
...
def main() -> None:
    ...
    try:
        ...
        # Add global error handler - this will catch all unhandled exceptions
